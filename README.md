...
1. I struggled in implementing the model on a single camera. The repository was not intuitive to follow and clone to be deployed on my chosen host, Streamlit. Relative import naming was a big hurdle to overcome, and even after that, the Streamlit serve is taking very long to implement the model (I say "is taking" because as of writing, I am still waiting for it to load). This will likely result in me being unable to implement and complete the question.
